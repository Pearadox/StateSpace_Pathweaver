...
public class Instrum {
	...
	public static void Process(TalonSRX tal, StringBuilder sb) {
		/* Smart dash plots */
		SmartDashboard.putNumber("SensorVel", tal.getSelectedSensorVelocity(Constants.ArmConstants.kPIDLoopIdx));
		SmartDashboard.putNumber("SensorPos", tal.getSelectedSensorPosition(Constants.ArmConstants.kPIDLoopIdx));
		SmartDashboard.putNumber("MotorOutputPercent", tal.getMotorOutputPercent());
		SmartDashboard.putNumber("ClosedLoopError", tal.getClosedLoopError(Constants.ArmConstants.kPIDLoopIdx));
		...
		if (_timesInMotionMagic > 10) {
			...
			SmartDashboard.putNumber("ClosedLoopTarget", tal.getClosedLoopTarget(Constants.ArmConstants.kPIDLoopIdx));
